...
var cpfClean = cleanCPF('049-214 3421-1');
cpfs.forEach(function( cpf ) {
    console.log ( cleanCPF( cpf ).replace( /(\d\d\d)(\d\d\d)(\d\d\d)(\d\d)/g , '$1.$2.$3-$4'));
});
/*
cpfClean.replace (/(\d{3})(\B)(\d{3})(\B)(\d{3})/g , function (regex, num, b, num2, b2, num3) {
...
"<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>".replace(/(<\w+>)(.*?)(<\/\w+>)/g, function(regex, tagAbertura, strn, tagFechar) {
    return tagAbertura + 'O texto dentro da tag "' + tagAbertura +'" é "' + strn + '"' + tagFechar + '\br';
});
})();